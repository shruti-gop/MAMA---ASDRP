...
class DataCreation():
    def __init__(self, title:str, tested_values: list):
        ...
        self.df= pd.DataFrame(columns=["answer_relevancy","faithfulness"],
        index=self.index)
        
    def add_data(self, research_paper:int, question:int, tested_value:int, answer_relevancy:float, faithfulness:float):
